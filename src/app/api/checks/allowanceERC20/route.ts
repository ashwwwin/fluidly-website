...
export async function GET(request: NextRequest) {
  try {
    ...
    const _allowance = (await _contract.allowance(wallet, operator)).toString();
    const allowance = parseInt(_allowance);

    return new NextResponse(JSON.stringify({ allowance }), {
      status: 200,
      headers: {
        "Content-Type": "application/json",
      },
